...
namespace se_core {
	...
	::CollisionGrid(coor_tile_t width, coor_tile_t height, short depth)
		...
		// Create node array
		nodes_ = new CollisionGridThingList::iterator_type[ totalNodeCount_ ];
		// Init nodes to empty
		for(int i = 0; i < totalNodeCount_; ++i) {
			nodes_[i] = CollisionGridThingList::end();
		}

		// Create and init array with pointers to
		// beginning of each node level
		nodeLevels_ = new CollisionGridThingList::iterator_type*[ depth_ ];
		int nodePos = 0;
		for(short i = 0; i < depth_; ++i) {
		...
		for(short level = 0; level < depth_; ++level) {
			...
			for(coor_tile_t z = zNodeFrom; z <= zNodeTo; ++z) {
				for(coor_tile_t x = xNodeFrom; x <= xNodeTo; ++x) {
					...
					while(it != CollisionGridThingList::end()) {
						Assert(count < max - 1);
						things[ count++ ] =
							thingList().next(it);
					}
