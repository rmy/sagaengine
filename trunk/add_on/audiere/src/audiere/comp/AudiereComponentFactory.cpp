...
#include "AudiereComponentFactory.hpp"
...
namespace se_audiere {
	...
	AudiereComponentFactory
	::AudiereComponentFactory()
		: ComponentFactory(sct_AUDIERE) {
	}


	Component* AudiereComponentFactory
	::create(Composite* owner) const {
		...
		AudiereComponent::Ptr pAudiere(*owner);
		if(pAudiere == 0) {
			// Create new component
			pAudiere = new AudiereComponent(owner, this);
		}
