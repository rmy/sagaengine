	...
	namespace PulseaudioSchema {
		...
		struct AutoInit : public se_core::InitListener {
			AutoInit() {
				...
				LogDetail("Registered Pulseaudio add-on");
			}

			~AutoInit() {
				SimSchema::initListeners().removeListener(*this);
				LogDetail("Cleaned up Pulseaudio add-on");
			}
