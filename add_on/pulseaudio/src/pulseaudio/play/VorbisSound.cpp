...
namespace se_pulseaudio {
  ...
  VorbisSound::VorbisSound(const char* filename) : ogg_(0), oggSize_(0) {
  ...
  VorbisSound::VorbisSound(FILE* infile) : ogg_(0), oggSize_(0) {
    init(infile);
  }
  ...
  int VorbisSound::close(OggVorbis_File* vf, int &current_section) {
    ov_clear(vf);
    current_section = 0;
    return 0;
  }
