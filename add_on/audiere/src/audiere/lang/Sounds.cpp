...
namespace se_audiere {
	...
	::add(unsigned short language, SoundType type, String* nameC, float volume, String* soundC, bool shouldLoop) {
		...
		audiere::OutputStreamPtr sound = AudiereSchema::soundPlayer->loadSound(soundC->get(), type != SOUND);
		if(!sound) {
		...
		AssertFatal(AudiereSchema::soundPlayer, "Sound player object not created");
