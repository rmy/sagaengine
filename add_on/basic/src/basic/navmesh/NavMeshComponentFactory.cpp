...
#include "NavMeshComponentFactory.hpp"
...
namespace se_basic {
	...
	::NavMeshComponentFactory()
		: ComponentFactory(sct_NAVMESH) {
	}


	Component* NavMeshComponentFactory
	::create(Composite* owner) const {
		...
		NavMeshComponent::Ptr pNavMesh(*owner);
		if(pNavMesh == 0) {
			// Create new component
			pNavMesh = new NavMeshComponent(owner, this);
		}
